...
static void Button_check(BUTTON *button){
	...
	if ((state == button->_getActiveState())
		...
		&& (button_debouncer.isDone()))
	{
		button_debouncer.start();
		//button->PREVSTATE = button->ACTIVE;
...
/*-------------------------------------Class ButtonDebouncer start-------------------------*/
void ButtonDebouncer::configure(){
    timer_->PSC = timer_prescaler_; 			// divides clock by ...
    timer_->SR = 0;							// clear interrupt flags
    timer_->CR1 |=  TIM_CR1_OPM;			// downcounter, one pulse mode TIM_CR1_DIR |
    timer_->ARR = debounce_time_;
    timer_->CR1 |= TIM_CR1_CEN;			
}

/*-------------------------------------Class ButtonDebouncer end---------------------------*/

/*-------------------------------------Class BUTTON start----------------------------------*/
BUTTON::BUTTON(GPIOPIN gpio_pin, uint8_t active_state, uint16_t long_press_time)
    : gpio_pin_(gpio_pin), active_state_(active_state), long_press_time_(long_press_time), prevstate(!active_state_){
